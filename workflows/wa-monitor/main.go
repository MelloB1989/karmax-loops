...
func monitor() error {
	...
	doPass := func(justReplied bool) error {
		...
		policy := "   - LEAN TOWARD REPLYING. If a reply/action is routine and you're reasonably sure how the operator would respond (acknowledgements, answering things you know from context, simple scheduling, sharing already-known info, confirming availability, a natural conversational reply), SEND IT NOW: `" + wacli + " send --to " + chatID + " --text \"...\"` " + voice + ". Use the `gog` CLI for calendar/email if clearly asked. When in doubt between replying and staying silent, REPLY.\n" +
		...
		if commanded {
			...
			policy = "   - Read the FULL context: the new message AND, for a reply, the quoted text it is responding to.\n" +
				"   - If it's an instruction/request/question you can handle (find something, do X, send Y, answer a question) — CARRY IT OUT FULLY using your tools/shell (research the web, run commands, use gog/gh, generate the answer), then POST the result in THIS chat via `" + wacli + " send --to " + chatID + " --text \"...\"` (use `--media <path>` if a file is wanted). Do the actual work, don't just acknowledge.\n" +
